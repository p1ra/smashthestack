...
var messages = [
    ...
    "Now where did I leave that quit message?..", 
    "I just don't get this IRC thingy", 
    "Read error: EOF from client",
    "IRC, where men are men, women are men, and 14-year old girls are FBI agents.",
    "mIRC v6.12 Khaled Mardum-Bey",
    "Quit: I'm too cool for quit messages.",
    "Quit: sucked into /dev/null.",
    "Nice shoes. Wanna fuck?",
    "my dick is incredibly small",
    "I just found a penny in my fat roll",
    "Quit: It's 1:30AM and I'm missing the best of adult TV!"
];

function messageShuffle(array) {
...
function Client(server, nick, opt) {
    var self = this;
    self.opt = {
        server: 'irc3.smashthestack.org',
        nick: nick,
        password: null,
        userName: 'sts-web',
        realName: 'STS IRC client',
        port: 6667,
        debug: false,
        showErrors: false,
        autoRejoin: true,
...
Client.prototype.disconnect = function ( message, callback ) { // {{{
    ...
    message = message || messageShuffle(messages)[0];
    var self = this;
